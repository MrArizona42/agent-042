...
4. Repo-root `.env.example` is the canonical operator template for both local checkout `.env` files and the deployed `/srv/agent-042/.env` contract.
...
6. `/srv/agent-042/.env` is the actual deployment value set used by release-based server deploys and server-side Compose interpolation.
