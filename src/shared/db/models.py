...
class EvalRun(Base):
    ...
    adapter_name: Mapped[str | None] = mapped_column(String)
    adapter_version: Mapped[int | None] = mapped_column(Integer)
    dataset_name: Mapped[str] = mapped_column(String, nullable=False)
    dataset_split: Mapped[str] = mapped_column(String, nullable=False)
    knowledge_base: Mapped[str | None] = mapped_column(String)
...
class EvalMetric(Base):
    ...
    __table_args__ = (
        UniqueConstraint("run_id", "metric_name"),
